...
},{"./map.js":6}],4:[function(require,module,exports){
'use strict';
...
var configs = {
  MAP_KEY: 'AIzaSyAqLHfe43F-5Vw_dHwgO8gi_Wt_mSAdwkA',
  URLS: {
    BASE: 'https://digitaslbi-id-test.herokuapp.com',
    BUS_STOPS: '/bus-stops'
  },
  MESSAGES: {
  ...
  var map = {
    ...
    bindEvents: function bindEvents() {
      ...
      (0, _jquery2['default'])('.close').on('click', function () {
        (0, _jquery2['default'])('.infoBox').removeClass('visible');
        map.resetMarkersIcon();
      });

      (0, _jquery2['default'])('.search--box input[type=submit]').on('click', map.searchBox);
    },
    ...
    drawMap: function drawMap() {
      ...
      map.theMap.setOptions({ styles: _mapStyles.mapStyles });
    ...
    geolocalizeUser: function geolocalizeUser() {
    ...
    getPins: function getPins() {
      var URL = _configs.configs.URLS.BASE + _configs.configs.URLS.BUS_STOPS;
      _jquery2['default'].ajax({
        ...
        success: function success(data) {
          data.markers.forEach(function (p) {
            if (_underscore2['default'].findWhere(map.pins, { id: p.id }) == null) {
              map.pins.push(p);
            }
          });
          map.drawPins();
    ...
    clickOnPin: function clickOnPin(marker) {
      google.maps.event.addListener(marker, 'click', function () {
        ...
        var offsetX = (0, _jquery2['default'])(window).width() > 768 ? 0.25 : 0;
        var offsetY = (0, _jquery2['default'])(window).width() > 768 ? 0 : -0.25;
    ...
    resetMarkersIcon: function resetMarkersIcon() {
    ...
    showInfoBox: function showInfoBox(id, name) {
      var $infoBox = (0, _jquery2['default'])('.infoBox');
      $infoBox.find("h3").text(name);

      var URL = _configs.configs.URLS.BASE + _configs.configs.URLS.BUS_STOPS + '/' + id;
      _jquery2['default'].ajax({
        url: URL,
        jsonp: "callback",
        dataType: "jsonp",
        success: function success(busStop) {
    ...
    displayInfoMessages: function displayInfoMessages(serviceDisruptions) {
      // TODO : change display order to give more importance to criticalMessages
      (0, _jquery2['default'])('.messages').empty();
      for (var property in serviceDisruptions) {
        if (serviceDisruptions.hasOwnProperty(property)) {
          if (serviceDisruptions[property].length > 0) {
            (0, _jquery2['default'])('.messages').append('\n              <span class=' + property + '> ' + serviceDisruptions[property] + ' </span>\n            ');
          }
        }
      }
    },

    displayDeparturesTable: function displayDeparturesTable(arrivals) {
      var html = "";
      ...
      if (arrivals.length === 0) {
        html = '<h4> ' + _configs.configs.MESSAGES.NO_ARRIVALS + ' </h4>';
      }

      (0, _jquery2['default'])('.infoBox').find("tbody").empty().append(html);
    },

    searchBox: function searchBox() {
      var address = (0, _jquery2['default'])('.search--box input[type=text]').val() + " London UK";
...
},{"./configs":4,"./map-styles":5,"jquery":1,"underscore":2}]},{},[3])
